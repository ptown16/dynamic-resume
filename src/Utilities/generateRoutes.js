  ...
  for (const themeKey of Object.keys(themeData.themes)) {
    ...
    for (const page of pageData.pages) {
      ...
      for (const sectionName of page.cardSections) {
        ...
        if (!section) {
          throw new Error(`Card section of name ${sectionName} does not exist. (Specified in page ${page.name})`)
        }
        ...
        for (const card of section.cards) {
          if (card.link) {
            allRoutes.push(
              buildExpandedPage(allRoutes.length, theme, page, SelectedPage, card)
            )
          }
        }
      }
      ...
      allRoutes.push(
        <Redirect key={allRoutes.length} from={theme.link + page.link} to={theme.link + page.link} />
      )
      allRoutes.push(
    ...
    allRoutes.push(
      <Redirect key={allRoutes.length} from={theme.link} to={theme.link + "/home"} />
    )
  }
  return allRoutes
}
