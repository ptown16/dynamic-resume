...
function Header() {
  ...
  if (pageData.pages.length > 0) {
    for (const page of pageData.pages) {
      headerItems.push(
        <HeaderItem
          key={headerItems.length}
          name={page.name}
          link={theme.link + page.link}
        />
      );
    }
  }
  return (
    <div
      ...
      style={{
        ...
        boxShadow:
          colors.headerBackground === `rgba(1, 1, 1, 0)` ? "" : theme.shadow
      }}
    >
      <LogoBox
        size={50}
        logoColor={colors.logo}
        boxColor={colors.logoBackground}
      />
      <h3 className="logo-name" style={{ color: colors.logoText }}>
        PEYTON TANZILLO
      </h3>
      <div
        className={
          "header-items-container " + (isExpanded ? "" : "hide-mobile")
        }
      >
        {headerItems}
      </div>
